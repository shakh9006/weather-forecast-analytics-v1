...
def parse_openweathermap_forecast(parquet_data: io.BytesIO) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        daily_stats['load_date'] = datetime.now().strftime("%Y-%m-%d")
    ...
    if daily_stats_list:
        dead_line = datetime.now() + timedelta(days=3)
        ...
        logging.info(f"Combined DataFrame shape: {len(combined_df)}")
        return combined_df
    else:
        logging.warning("No daily stats data to process")
        return pd.DataFrame()
    
def parse_weatherbit_forecast(parquet_data: io.BytesIO) -> pd.DataFrame:
    df = pd.read_parquet(parquet_data)
    ...
    for _, row in df.iterrows():
        ...
        logging.info(f"City: {city}, Lat: {lat}, Lon: {lon}")
        ...
        weather_list['load_date'] = datetime.now().strftime("%Y-%m-%d")
...
def load_forceast_to_postgres(df: pd.DataFrame) -> None:
    ...
    try:
        ...
        if not existing_data:
            ...
            logging.info(f"Sample existing data: {existing_data[['city_name', 'load_date', 'forecast_date']].head()}")
            logging.info(f"Sample new data: {df_normalized[['city_name', 'load_date', 'forecast_date']].head()}")
            
            existing_keys = existing_data[["city_name", "load_date", "forecast_date"]].copy()
            existing_keys['exists'] = True
            
            merged = df_normalized.merge(
                existing_keys,
                on=["city_name", "load_date", "forecast_date"],
                how="left"
            )
